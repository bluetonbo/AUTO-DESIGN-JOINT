...
class PartConfig:
    ...
    xlsx_path: str                               # 학습 데이터 엑셀 경로
    header_row: int                              # 변수명이 있는 행 (0-index)
    spec_row: int                                # 스펙 텍스트가 있는 행 (0-index)
    data_row_start: int                          # 실측 데이터 시작 행 (0-index)
    data_row_end: int                            # 실측 데이터 끝 행 (exclusive, 0-index)
    kpi_columns: list[str]                       # KPI(목표) 컬럼명 리스트
    kpi_directions: dict[str, str]                # KPI별 "min"/"max"
...
# ------------------------------------------------------------------------------
# 데이터 로드 (엑셀 -> 변수/KPI 메타 + 정제된 데이터)
# ------------------------------------------------------------------------------

def load_part_data(cfg: PartConfig):
    """
    엑셀에서 데이터를 읽어:
    ...
    를 반환.
    """
    raw = pd.read_excel(cfg.xlsx_path, sheet_name=0, header=None)
